...
export class DashboardComponent implements OnInit {
  ...
  accesosRapidos = [
    { label: 'Gestionar técnicos',  icon: 'engineering',    route: '/tecnicos',  color: 'bg-accent/10 text-accent border-accent/20' },
    { label: 'Editar taller',        icon: 'store',          route: '/taller',    color: 'bg-accent-orange/10 text-accent-orange border-accent-orange/20' },
    { label: 'Ver turnos del día',   icon: 'calendar_month', route: '/turnos',    color: 'bg-success/10 text-success border-success/20' },
  ];
  ...
  ngOnInit(): void {
    ...
    if (t) {
      ...
      // ya sabemos que no hay taller, no hacer más llamadas
